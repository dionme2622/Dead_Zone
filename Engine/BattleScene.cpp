...
#pragma region PlayerCamera
	{
		...
		_playerCamera->AddComponent(make_shared<Transform>());
		_playerCamera->AddComponent(make_shared<Camera>());
		_playerCamera->GetTransform()->SetLocalPosition(Vec3(0.01f, 2.03f, -7.65f));
		_playerCamera->GetTransform()->LookAt(Vec3(0.f, 0.f, 1.f));
		uint8 layerIndex = LayerNameToIndex(L"UI");
		_playerCamera->GetCamera()->SetCullingMaskLayerOnOff(layerIndex, true); // UI는 안 찍음
		AddGameObject(_playerCamera);
...
#pragma region Directional Light
	{
		...
		light->GetTransform()->SetLocalPosition(Vec3(0, 1000.f, 0));
...
#pragma endregion
	...
	obj->AddComponent(meshRenderer);
	obj->AddComponent(make_shared<BoxCollider>(Vec3(0,0,0), Vec3(5,5,5)));										// Capsule Collider 생성
	obj->AddComponent(make_shared<RigidBody>(0.0f, dynamic_pointer_cast<BoxCollider>(obj->GetCollider()), Vec3(0,0,0), false));			// Rigid Body 생성
...
#pragma region Map
	...
	//{
	//	shared_ptr<MeshData> scene = GET_SINGLE(Resources)->LoadModelFromBinary(L"..\\Resources\\Model\\Wall3.bin"); // MeshData* meshData
	...
	//{
	//	shared_ptr<MeshData> scene = GET_SINGLE(Resources)->LoadModelFromBinary(L"..\\Resources\\Model\\Wall4.bin"); // MeshData* meshData
