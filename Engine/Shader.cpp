...
void Shader::CreateGraphicsShader(const wstring& path, ShaderInfo info, ShaderArg arg)
{
	...
	case SHADER_TYPE::SHADOW:
		...
		break;
	case SHADER_TYPE::SSAO:
		_graphicsPipelineDesc.NumRenderTargets = 1;
		_graphicsPipelineDesc.RTVFormats[0] = DXGI_FORMAT_R32_FLOAT; // SSAOTexture 포맷
		_graphicsPipelineDesc.DSVFormat = DXGI_FORMAT_UNKNOWN; // 깊이-스텐실 버퍼 사용 안 함
		break;
	}
